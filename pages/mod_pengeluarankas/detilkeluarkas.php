        <?php
          ...
          $query = $db->query("SELECT no_transaksi, tgl_transaksi, nominal, tb_ketkas.keterangan as ket, dest
                      FROM tb_kas, tb_ketkas where tb_kas.ket_transaksi=tb_ketkas.id_ket and tb_kas.status='keluar'");
          
          while ($row = $query->fetch(PDO::FETCH_ASSOC)) { 
        ?>
 
        <div <?php echo 'id="detilpkModal'.$row['no_transaksi'].'"' ?> class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h3 id="myModalLabel">Detil Pengeluaran Kas</h3>
              </div>
              <div class="modal-body">
                <form>
                  ...
                  <div class="form-group"><label>Bayar ke</label><input required class="form-control required text-capitalize" data-placement="top" data-trigger="manual" type="text" name="bayarke" value="<?php echo $row['dest']; ?>" readonly></div>
                </form>
              </div>
            </div>
          </div>
